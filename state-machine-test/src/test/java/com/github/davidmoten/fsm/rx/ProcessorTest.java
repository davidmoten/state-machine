...
public class ProcessorTest {
	...
	private static Processor<String> createProcessor(TestScheduler signalScheduler) {
		...
		Func2<Class<?>, String, EntityStateMachine<?, String>> stateMachineFactory = StateMachineFactory
				.cls(Microwave.class).<String> hasFactory(id -> MicrowaveStateMachine.create(Microwave.fromId(id), id,
						behaviour, MicrowaveStateMachine.State.READY_TO_COOK, Clock.from(signalScheduler)))
				.build();

		// build a processor
		Processor<String> processor = Processor.stateMachineFactory(stateMachineFactory)
				.processingScheduler(Schedulers.immediate()).signalScheduler(signalScheduler).build();
		return processor;
	}
