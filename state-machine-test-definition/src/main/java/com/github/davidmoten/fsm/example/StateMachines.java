...
public class StateMachines implements Supplier<List<StateMachine<?>>> {
	...
	private static StateMachine<Ship> createShipStateMachine() {
		...
		State<NextPortUnknown> departedToUnknownPort = m.createState("Departed To Unknown Port", NextPortUnknown.class);
		State<Risky> notifiedNextPort = m.createState("Notified Next Port", Risky.class);
		State<NotRisky> departedToPortNotRisky = m.createState("Departed To Port Not Risky", NotRisky.class);
		...
		neverOutside.initial()
				...
				.to(insideNotRisky).to(insideRisky.from(insideRisky).from(insideNotRiskyAlreadyNotified))
				.to(departed.from(insideNotRisky).from(insideNotRiskyAlreadyNotified)).to(departedToPort)
				.to(notifiedNextPort);
		return m;
	}
