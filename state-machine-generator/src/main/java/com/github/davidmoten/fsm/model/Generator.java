...
public final class Generator<T> {
	...
	private boolean hasCreationTransition() {
		return machine.transitions().stream().filter(t -> t.from().isInitial()).findAny().isPresent();
	}
	...
	private void generateStateMachine() {
		...
		try (PrintStream out = new PrintStream(bytes)) {
			...
			out.format("%sreturn new %s(%s, behaviour, state);\n", indent, stateMachineClassSimpleName(),
					instanceName());
			out.format("%s}\n", indent.left());
			out.println();
			...
			Stream.concat(states().filter(state -> state.isInitial()).map(state -> state.eventClass()),
